...
public final class AnnotationUtil {
     ...
     * @throws IllegalAccessException
     */
    public static Map<Class<? extends Annotation>, Annotation> getAnnotationData(final Method method) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        Method target = Executable.class.getDeclaredMethod("declaredAnnotations");
        target.setAccessible(true);
        return (Map<Class<? extends Annotation>, Annotation>) target.invoke(method);
    }
    ...
    public static boolean isJdkAnnotation(final Annotation annotation) {
        return isJdkAnnotation(annotation.annotationType());
    }
    /**
     * 判断当前对象是否复核注解的目标添加对象类型
     * @param element
     * @param annotation
     * @return
     */
    public static boolean isTargetType(AnnotatedElement element, Annotation annotation) {
        Target target = annotation.annotationType().getAnnotation(Target.class);
        if (target == null) return false;

        // TODO 根据注解的target属性限制element类型
        ElementType[] types = target.value();
        for (ElementType type : types) {
            switch (type){
                case PACKAGE: break;
                case TYPE:
                    if (element instanceof Class) return true;
                    break;
                case TYPE_USE: break;
                case TYPE_PARAMETER: break;
                case ANNOTATION_TYPE:
                    if (((Class)element).isAnnotation()) return true;
                    break;
                case FIELD:
